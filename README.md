# Auto Code Marker – Version 1
Programming Assignment Testing Tool  
CS 2043 – Group 1  
Members: Alamin Adeleke, Aniekan Ekarika, Chukwunonso Ekweaga, Frances Felicidario

## Overview
This project is a JavaFX-based tool that automates running student Java submissions against instructor-defined test cases.

## Features (Version 1)
- Import / create test cases and suites  
- Execute test suites on multiple submissions  
- Display pass/fail results  
- Save and load test data 
